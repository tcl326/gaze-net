...
train_data = trainGenerator.flow_from_directory(dataset_path, subset='training',time_steps=ts, batch_size=bs, 
                                                target_size=(img_size,img_size), crop=True, gaussian_std=3)
...
for i in range(ts):
	...
	y = gaze[2]
	left = int(max(0, x-10))
	right = int(min(x+10, img_size-1))
	above = int(max(0, y-10))
	bottom = int(min(y+10, img_size-1))
	img[above:bottom, left:right, 0] = 255
	img[above:bottom, left:right, 0] = 0
	img[above:bottom, left:right, 0] = 0
	img_name = test_path + str("%3d" % i) + '.png'
	plt.imsave(img_name, img)

# visualize for crop with gaze
# train_data = trainGenerator.flow_from_directory(dataset_path, subset='training',time_steps=ts, 
#                                                 batch_size=bs, crop=True, target_size=(360,360),
#                                                 gaussian_std=0.01, time_skip=1, crop_with_gaze=True,
